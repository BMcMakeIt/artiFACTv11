...
class ClassifierApp:
    ...
    def _show_species_candidates(self, cands: list):
        ...
        try:
            cands = [c for c in (cands or []) if _is_species_name(
        ...
        def _select(val, cand):
            ...
            store_label = sci if (elem in WHOLE_ELEMENTS or not elem) else (
                f"{elem} — {sci}" if sci else elem)
            self.last_classification = {
                "label": store_label, "confidence": cand.get("confidence", 0)}
            # Reference images:
        ...
        if cands:
            first_clean = self._format_taxon_label(
                cands[0], include_conf=False)
            var.set(first_clean)
