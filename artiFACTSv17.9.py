...
def choose_discogs_release(results: list) -> int:
    ...
    SlateLabel(frame, text="Select the correct release:").pack(
        anchor='w', pady=(0, 6))
...
AVATAR_PATHS = {
    "mineral": "assets/avatars/mineral.png",
    "shell": "assets/avatars/shell.png",
    "fossil": "assets/avatars/fossil.png",
    "zoological": "assets/avatars/zoological.png",
    "coin": "assets/avatars/coin.png",
    "vinyl": "assets/avatars/vinyl.png",
    "other": "assets/avatars/other.png",
}


def _load_avatar_for_category(cat: str, size=(60, 60)):
    """Return an ImageTk.PhotoImage for the avatar if available, else None."""
    try:
        path = AVATAR_PATHS.get(
            (cat or "other").strip().lower()) or AVATAR_PATHS["other"]
...
def _first_photo_for_item(detail_map: dict, fallback_path: str) -> str:
    ...
    if not any(paths) and upload and os.path.exists(upload):
        paths[0] = upload
...
class BannerMixin:
    def add_banner(self, container, height=64):
        ...
        canvas.bind('<Configure>', lambda e: draw_banner(
    ...
    def _draw_blurb(self, text):
        ...
        try:
            ...
            if getattr(self, "_current_item", None):
                cat = (self._current_item.get(
                    "category") or "").strip().lower()

            avatar_img = _load_avatar_for_category(cat, size=(120, 120))
            if avatar_img:
                # keep a reference so Tkinter doesn't GC it
                if not hasattr(self, "_avatar_refs"):
                    self._avatar_refs = []
                self._avatar_refs.append(avatar_img)

                expert = tk.Label(frame, image=avatar_img, bg=frame.cget('bg'))
                expert.image = avatar_img  # extra belt/suspenders
                expert.pack(side='left', padx=(0, 10))
            else:
                # fallback to the existing silhouette if no image found
        ...
        if self._current_item:
            ...
            if current_id != self._blurb_item_id or not self.current_blurb_text:
                ...
                self.current_blurb_text = get_expert_blurb(
                    ...
                    data.get('name'),
                    data.get('details') or {},
                    (data.get('details') or {}).get('img1_path')
                    or (data.get('row') and data.get('row')[data.get('cols').index("photo_path")]
                        if data.get('cols') and data.get('row') else "")
                )

        # *** Fix: update _blurb_item_id before saving ***
                self._blurb_item_id = current_id
                save_blurb_to_db(current_id, self.current_blurb_text)
    ...
    def go_review(self):
        ...
        if not self.images:
            messagebox.showwarning(
                "Missing photo", "Choose at least one image before review.")
            return
...
def build_front_hub(parent, on_add_item, on_help_ident, on_open_library):
    ...
    SlateButton(top, text="I Know What My Artifact Is",
                command=on_add_item).pack(side='left')
    SlateButton(top, text="Identify Artifact With AI", command=on_help_ident).pack(
        side='left', padx=6)
    SlateButton(top, text="Open Library", command=on_open_library).pack(
        side='right', padx=8)

    # Title for carousel
    SlateTitle(parent, text="Your Collection").pack(
...
class ItemDetailWindow(tk.Toplevel, BannerMixin):
    ...
    def scan_barcode(self, field_key: str = "barcode_ean_upc"):
        ...
        status = SlateLabel(
            frame, text="Align barcode in the box. Press Capture (or Space).")
...
if __name__ == '__main__':
    ...
    banner.bind('<Configure>', lambda e: draw_banner(
        banner, e.width, e.height))
